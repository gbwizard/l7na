...
static unsigned int off_ocontrol;
static unsigned int off_opos;
static unsigned int off_istatus;
static unsigned int off_ipos;

const static ec_pdo_entry_reg_t domain1_regs[] = {
    {0, 0, 0x00007595, 0x00000000, 0x6040, 0, &off_ocontrol},
    {0, 0, 0x00007595, 0x00000000, 0x607a, 0, &off_opos},
    {0, 0, 0x00007595, 0x00000000, 0x6041, 0, &off_istatus},
    {0, 0, 0x00007595, 0x00000000, 0x6064, 0, &off_ipos},
    {}
};
...
int main(int argc, char **argv)
{
    ...
    for(j = 0; ; j++) {
       ...
       ipos = EC_READ_U32(domain1_pd + off_ipos); //READ DATA 0x6064 position
       istatus = EC_READ_U16(domain1_pd + off_istatus); //READ DATA 0x6041 status
    ...
    if(argc > 0) {
        ...
        cmd = 15;
        EC_WRITE_U16(domain1_pd + off_ocontrol, cmd);
        ...
        ecrt_domain_process(domain1);
        val = cmdpos;
        EC_WRITE_U32(domain1_pd + off_opos, val);
        ecrt_domain_queue(domain1);
        ecrt_master_send(master);
        usleep(1000);
